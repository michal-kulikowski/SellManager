...
def lokalizacje_list(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            dom_sql_main = '''SELECT MIN(lokalizacja.ID_ADR_DOM_PODPISUJACY) AS id_adr_dom_podpisujacy, powiat.nazwa_powiatu, wojewodztwo.nazwa_wojewodztwa,
            gmina.NAZWA_GMINY, miejscowosc.NAZWA_MIEJSCOWOSCI, ulica.TYP, ulica.NAZWA_ULICY, kod.KOD, dom.NUMER_DOMU, MIN(symbol.SYMBOL) AS SYMBOL,
            ...
            LEFT JOIN USORT4.ADR_KOD_POCZTOWY kod ON dom.ID_ADR_KOD_POCZTOWY = kod.ID_ADR_KOD_POCZTOWY
            ...
            INNER JOIN USORT4.ADR_POWIAT powiat ON gmina.ID_ADR_POWIAT = powiat.ID_ADR_POWIAT  
            INNER JOIN USORT4.ADR_WOJEWODZTWO wojewodztwo ON powiat.ID_ADR_WOJEWODZTWO = wojewodztwo.ID_ADR_WOJEWODZTWO
            ...
            INNER JOIN USORT4.WAZNOSCI_PORTOW waznosc_portu ON oplata.ID_OPLATY_STALE = waznosc_portu.ID_OPLATY_STALE
             ...
             OR TRUNC(oplata.KON_WAZNOSCI) >= TRUNC(CURRENT_DATE)) AND TRUNC(waznosc_portu.POCZ_WAZNOSCI) <= TRUNC(CURRENT_DATE)
              ...
              GROUP BY sprzedawca.ID_SPRZEDAWCA, dom.ID_ADR_DOM) klienci ON lokalizacja.ID_SPRZEDAWCA = klienci.ID_SPRZEDAWCA
               ...
               GROUP BY gmina.NAZWA_GMINY, powiat.nazwa_powiatu, wojewodztwo.nazwa_wojewodztwa, miejscowosc.NAZWA_MIEJSCOWOSCI, ulica.TYP, ulica.NAZWA_ULICY, dom.NUMER_DOMU, budynek.NAZWA_TYPU,
               dom.LICZ_LOKALI, dom.PREDKOSC_MAX, handlowiec.nazwisko, technologie.TECHNOLOGIE, kod.kod'''
